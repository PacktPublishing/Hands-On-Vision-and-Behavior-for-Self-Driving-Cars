...
def generator(ids, fn_image, fn_label, enable_data_augmentation, batch_size):
    num_samples = len(ids)
    while 1:  # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            batch_samples = np.array([fn_image(x, enable_data_augmentation and (idx % 2 == 0)) for idx, x in enumerate(batch_samples_ids)])
            batch_labels = np.array([fn_label(x, enable_data_augmentation and (idx % 2 == 0)) for idx, x in enumerate(batch_samples_ids)])

            yield batch_samples, batch_labels


def extract_image(file_name, mirror):
...
def extract_label(file_name, mirror):
...
def dn_dense(layer, growth_rate_k, num_layers, add_bottleneck_layer, dropout=0.0):
    block_layers = []
    for i in range(num_layers):
        new_layer = dn_conv(layer, 4 * growth_rate_k, (1, 1), dropout) if add_bottleneck_layer else layer
        new_layer = dn_conv(new_layer, growth_rate_k, (3, 3), dropout)
...
def dn_transition_up(skip_connection, layer):
    assert 0.0 < expansion_factor <= 1.0
    num_filters_expanded = int(layer.shape[-1])
    layer = Conv2DTranspose(num_filters_expanded, kernel_size=3, strides=2, padding='same',
...
def fc_dense_net(input_shape, dropout=0.0):
    ...
    # Output
    layer = Conv2D(num_classes, kernel_size=1, padding='same', kernel_initializer='he_uniform')(layer)
    # layer = Reshape((-1, num_classes))(layer)
    output = Activation('softmax')(layer)

    return Model(input, output)


model = fc_dense_net((size[0], size[1], 3), 0.2)
