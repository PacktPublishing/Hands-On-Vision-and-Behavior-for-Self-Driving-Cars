...
for filename in os.listdir("in_calibration"):
    image = cv2.cvtColor(cv2.imread("in_calibration/" + filename), cv2.COLOR_BGR2GRAY)
    found, corners = cv2.findChessboardCorners(image, (nY, nX), None)

    print(filename, ": ", found, image.shape)
    if found:
        ...
        img_corners = cv2.drawChessboardCorners(image, (nY, nX), corners, True)
...
for filename in os.listdir("in_calibration"):
    ...
    cv2.imwrite('out_calibrated/' + filename,dst)

for filename in os.listdir("test_calibration"):
    image = cv2.imread("test_calibration/" + filename)
    dst = cv2.undistort(image, mtx, dist, None, optimal_matrix)
    cv2.imwrite('out_calibrated/' + filename,dst)
